...
import { Graph, type Profile } from 'near-social-js'
...
export function useGraphInstance() {
  const { social } = useWallet()
  return useMemo(
    () => social ?? new Graph({ network: 'mainnet' }),
    [social]
  )
}

export function useGraphGet(keys: string[], options?: Omit<UseQueryOptions<unknown>, 'queryKey' | 'queryFn'>) {
...
export function useGraphKeys(patterns: string[], options?: Omit<UseQueryOptions<unknown>, 'queryKey' | 'queryFn'>) {
  const graph = useGraphInstance()
...
export function useGraphVersion(options?: Omit<UseQueryOptions<unknown>, 'queryKey' | 'queryFn'>) {
  const graph = useGraphInstance()
...
export function useGraphAccount(accountId: string, options?: Omit<UseQueryOptions<unknown>, 'queryKey' | 'queryFn'>) {
  const graph = useGraphInstance()
...
export function useGraphAccountCount(options?: Omit<UseQueryOptions<unknown>, 'queryKey' | 'queryFn'>) {
  const graph = useGraphInstance()
...
export function useGraphStorageBalance(accountId: string, options?: Omit<UseQueryOptions<unknown>, 'queryKey' | 'queryFn'>) {
  const graph = useGraphInstance()
...
export function useSetProfile() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      if (accountId) {
        queryClient.invalidateQueries({ queryKey: socialKeys.profile(accountId) })
      }
      queryClient.invalidateQueries({ queryKey: socialKeys.all })
    },
  })
